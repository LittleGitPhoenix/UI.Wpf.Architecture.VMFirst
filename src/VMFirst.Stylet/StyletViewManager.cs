...
public class StyletViewManager : IViewManager
{
	...
	{
		// This is necessary for "Command="{s:Action ...}" to properly function.
		View.SetActionTarget(view, model);

		// This is necessary for the stylet IViewAware interface to properly function. This interface is implemented by many stylet classes (Screen ,Conductor, ...) and doing this is therefore mandatory.
		if (model is IViewAware styletViewAwareModel) styletViewAwareModel.AttachView(view);
	}
